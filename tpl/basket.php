...
<div id="table-actions">
    ...
    // товары есть
        ...
        <?php }
				...
				// учтём скидки
				$day2price_discount = check_discount_percentage_get($day2price, $order['discount_input']);
				$day2price -= $day2price_discount;
				$day3price_discount = check_discount_percentage_get($day3price, $order['discount_input']);
				$day3price -= $day3price_discount;
    ...
    <!-- скидки -->
        <tr>
            ...
            <td colspan="3">
                <!-- поле ввода скидки -->
                <input type="text" id="discount_input" name="discount_input" value="<?php echo isset($order['discount_input']) ? h($order['discount_input']) : '0'; ?>">
            </td>
						
            <td class="main-text">
                <!-- стоимость скидки -->
                <?php echo h($order['discount_value']); ?>
            </td>
        </tr>
...
';
	...
	if($order['discount_input'] != 0) {
		echo '💵 Итого без скидки: ' . number_format($order['full_price'], 0, '', ' ') . ' ₽' . "\r\n";
		echo "\r\n" . '🔖 Скидка: ';
		echo (is_string($order['discount_input']) && strpos($order['discount_input'], '%') !== false) ? $order['discount_input'] . ' = ' : '';
		echo $order['discount_value'] . ' ₽';
		echo "\r\n" . '💸 Итого c учётом скидки: ' . number_format($order['price'], 0, '', ' ') . ' ₽';
	} else {
...
Карта Сбербанк на имя Алексей Дмитриевич К. привязана к номеру ' . $admin['telephone_bank']; ?>
...
<?php if($order['upfront'] != '0') {
	...
	if($order['upfront'] == $order['price']) {
		...
		echo "\r\n" . '💵 К оплате при получении: ' . number_format( ( $order['price'] - $order['upfront'] ), 0, '', ' ' ) . ' ₽'; if( $order['customer_returning'] != 1 ) { echo ' + залог.'; } echo "\r\n";
	}
}
?>
...
<?php echo '📲 Просим предупредить о визите за час.';?>
...
<?php echo '❤️ ' . (not_empty($order['customer_name']) ? $order['customer_name'] . ', благодарим' : 'Благодарим') . ' за бережное обращение со снаряжением. При следующих арендах залог не требуется.' . "\r\n"; ?>
...
<?php echo '💼 При оплате по безналичному расчёту для юридических лиц стоимость увеличивается на 6%.';?>
