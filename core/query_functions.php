...
function find_oldest_cancelled_order($admin_id) {
	global $db;

	$sql = "SELECT id FROM orders ";
	$sql .= "WHERE status_id='0' "; // статус 0 означает "отменён"
	$sql .= "AND admin_id='" . db_escape($db, $admin_id) . "' ";
...
function create_order($admin_id, $order) {
  global $db;
	...
	$sql .= "(admin_id, inventory, discount_input, discount_value, duration, price, full_price, deposit, customer_returning, customer_name, customer_tel, customer_address, comment, upfront) ";
	$sql .= "VALUES (";
	$sql .= "'" . db_escape($db, $admin_id) . "', ";
	$sql .= "'" . db_escape($db, $order['inventory']) . "', ";
	$sql .= "'" . db_escape($db, $order['discount_input']) . "', ";
	$sql .= "'" . db_escape($db, $order['discount_value']) . "', ";
	$sql .= "'" . db_escape($db, $order['duration']) . "', ";
	$sql .= "'" . db_escape($db, $order['price']) . "', ";
	$sql .= "'" . db_escape($db, $order['full_price']) . "', ";
...
function update_order($order) {
	...
	$sql .= "discount_input='" . db_escape($db, $order['discount_input']) . "', ";
	$sql .= "discount_value='" . db_escape($db, $order['discount_value']) . "', ";
	$sql .= "duration='" . db_escape($db, $order['duration']) . "', ";
	$sql .= "price='" . db_escape($db, $order['price']) . "', ";
	$sql .= "full_price='" . db_escape($db, $order['full_price']) . "', ";
